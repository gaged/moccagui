...
{
  if (axis == 0) {
    return emcStatus->motion.traj.position.tran.x - emcStatus->task.g5x_offset.tran.x - emcStatus->task.g92_offset.tran.x;
  } else 
  if (axis == 1) {
    return emcStatus->motion.traj.position.tran.y - emcStatus->task.g5x_offset.tran.y - emcStatus->task.g92_offset.tran.y;
  } else 
  if (axis == 2) {
    return emcStatus->motion.traj.position.tran.z - emcStatus->task.g5x_offset.tran.z - emcStatus->task.g92_offset.tran.z;
  } else 
  if (axis == 3) {	
    return emcStatus->motion.traj.position.a - emcStatus->task.g5x_offset.a - emcStatus->task.g92_offset.a;
  } else 
  if (axis == 4) {	
    return emcStatus->motion.traj.position.b - emcStatus->task.g5x_offset.b - emcStatus->task.g92_offset.b;
  } else 
  if (axis == 5) {
    return emcStatus->motion.traj.position.c - emcStatus->task.g5x_offset.c - emcStatus->task.g92_offset.c;
  } else 
  if (axis == 6) {
    return emcStatus->motion.traj.position.u - emcStatus->task.g5x_offset.u - emcStatus->task.g92_offset.u;
  } else 
  if (axis == 7) {
    return emcStatus->motion.traj.position.v - emcStatus->task.g5x_offset.v - emcStatus->task.g92_offset.v;
  } else 
  if (axis == 8) {	
    return emcStatus->motion.traj.position.w - emcStatus->task.g5x_offset.w - emcStatus->task.g92_offset.w;
  } else {
    return 0;
  }
}
...
{
  if (axis == 0) {
    return emcStatus->motion.traj.actualPosition.tran.x - emcStatus->task.g5x_offset.tran.x - emcStatus->task.g92_offset.tran.x;
  } else 
  if (axis == 1) {
    return emcStatus->motion.traj.actualPosition.tran.y - emcStatus->task.g5x_offset.tran.y - emcStatus->task.g92_offset.tran.y;
  } else 
  if (axis == 2) {
    return emcStatus->motion.traj.actualPosition.tran.z - emcStatus->task.g5x_offset.tran.z - emcStatus->task.g92_offset.tran.z;
  } else 
  if (axis == 3) {	
    return emcStatus->motion.traj.actualPosition.a - emcStatus->task.g5x_offset.a - emcStatus->task.g92_offset.a;
  } else 
  if (axis == 4) {	
    return emcStatus->motion.traj.actualPosition.b - emcStatus->task.g5x_offset.b - emcStatus->task.g92_offset.b;
  } else 
  if (axis == 5) {
    return emcStatus->motion.traj.actualPosition.c - emcStatus->task.g5x_offset.c - emcStatus->task.g92_offset.c;
  } else 
  if (axis == 6) {
    return emcStatus->motion.traj.actualPosition.u - emcStatus->task.g5x_offset.u - emcStatus->task.g92_offset.u;
  } else 
  if (axis == 7) {
    return emcStatus->motion.traj.actualPosition.v - emcStatus->task.g5x_offset.v - emcStatus->task.g92_offset.v;
  } else 
  if (axis == 8) {	
    return emcStatus->motion.traj.actualPosition.w - emcStatus->task.g5x_offset.w - emcStatus->task.g92_offset.w;
  } else {
    return 0;
  }
}
...
{
  if (axis == 0) {
    return emcStatus->task.g5x_offset.tran.x - emcStatus->task.g92_offset.tran.x;
  } else
  if (axis == 1) {
    return emcStatus->task.g5x_offset.tran.y - emcStatus->task.g92_offset.tran.y;
  } else
  if (axis == 2) {
    return emcStatus->task.g5x_offset.tran.z - emcStatus->task.g92_offset.tran.z;
  } else
  if (axis == 3) {
    return emcStatus->task.g5x_offset.a - emcStatus->task.g92_offset.a;
  } else
  if (axis == 4) {
    return emcStatus->task.g5x_offset.b - emcStatus->task.g92_offset.b;
  } else
  if (axis == 5) {
    return emcStatus->task.g5x_offset.c - emcStatus->task.g92_offset.c;
  } else
  if (axis == 6) {
    return emcStatus->task.g5x_offset.u - emcStatus->task.g92_offset.u;
  } else
  if (axis == 7) {
    return emcStatus->task.g5x_offset.v - emcStatus->task.g92_offset.v;
  } else
  if (axis == 8) {
    return emcStatus->task.g5x_offset.w - emcStatus->task.g92_offset.w;
  } else {
    return 0;
  }
}
