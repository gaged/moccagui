unit simulator;

{$mode objfpc}{$H+}

interface

uses
  Classes, ComCtrls, Menus, StdCtrls, SysUtils, FileUtil, LResources, Forms,
  Controls, Graphics, Dialogs, ExtCtrls,
  glu,gl, mocglb;

type
  TSingle2dArray = array of array of Single;

type
  PPlaneArray = ^TPlaneArray;
  TPlaneArray = Array[-1024..1024] of Integer;

type
  TToolDef = record
    Size: integer;
    PA: PPlaneArray;
  end;

type

  { TSimulatorDlg }

  TSimulatorDlg = class(TForm)
    Bevel1: TBevel;
    BtnRender: TButton;
    BtnCancel: TButton;
    cbSample: TComboBox;
    EditTool: TEdit;
    Label1: TLabel;
    Label2: TLabel;
    Label3: TLabel;
    LabelStatus: TLabel;
    ProgressBar: TProgressBar;
    procedure BtnCancelClick(Sender: TObject);
    procedure BtnRenderClick(Sender: TObject);
    procedure FormCreate(Sender: TObject);
    procedure FormDestroy(Sender: TObject);
  private
    DefaultToolDia: Single; // default tool diameter in user units
    CountX,CountY: integer; // The dimensions of the array
    MinX: Double; // the left position of the x0 array item
    MinY: Double; // the bottom position of the y0 array item
    NA: TSingle2DArray; // the 2d array of single- values for the z-height map
    Res: Double;  // resolution of heightmap
    LightPosition: Array[0..3] of glFloat;
    ToolDef: TToolDef;
    SampleRate: integer;
    Initialized: Boolean;
    procedure ArrayToPos(i,k: integer; var x,y: double);
    procedure CreateTool(ADia: Single);
    procedure CutLine(n1,n2: tlo);
    procedure CutAt(x,y: integer; z: double);
    //procedure Lightning;
    procedure MakeList;
    procedure InitRender;
    procedure Render;
    procedure UpdateEdit;
    procedure SetTool(ADia: Double);
  public
    GlList: gluInt;
  end;

function Show3DPreviewDlg(var AGlList: gluInt): Boolean;

implementation

uses
  math,gllist;

var
  Rendering: Boolean;
  DoAbort: Boolean;

function Show3DPreviewDlg(var AGlList: gluInt): Boolean;
var
  Dlg: TSimulatorDlg;
begin
  Result:= False;
  if not Assigned(Renderer) then
    Exit;
  Application.CreateForm(TSimulatorDlg,Dlg);
  try
    Rendering:= False;
    Dlg.GlList:= AGlList;
    Dlg.ShowModal;
  finally
    Dlg.Free;
  end;
  Result:= True;
end;

procedure TSimulatorDlg.CutAt(x,y: integer; z: double);
var
  c,i,k: integer;
  OldZ: Single;
  ix,iy: integer;
  tx,ty: integer;
  r: integer;
begin
 with ToolDef do
   begin
     if PA = nil then
       raise Exception.create('Cannot Render without a defined tool');
     r:= Size div 2;
     for iy:= -r to r do
       begin
         c:= PA^[iy];
         if (c > 0) and (c <= r) then
           for ix:= -c to c do
             begin
               tx:= ix + x;
               ty:= iy + y;
               if (tx >= 0) and (ty >= 0) then
                 if (tx < CountX) and (ty < CountY) then
                   begin
                     OldZ:= NA[tx,ty];
                     if z < OldZ then
                     NA[tx,ty]:= z;
                   end;
             end;
       end;
  end;
end;

procedure TSimulatorDlg.CutLine(n1,n2: tlo);
var
  x1,y1,x2,y2: integer;
  dx,dy,dz: double;
  t1: double;
  nx,ny,t: integer;
  ToolRad: integer;
begin
  x1:= Round((n1.x - MinX) / res);
  y1:= Round((n1.y - MinY) / res);
  x2:= Round((n2.x - MinX) / res);
  y2:= Round((n2.y - MinY) / res);

  if(x1 = x2) and (y1 = y2) then
    begin
      CutAt(x1,y1,Min(n1.z,n2.z));
      Exit;
    end;

  dx:= x2 - x1;
  dy:= y2 - y1;
  dz:= n2.z - n1.z;
  t1:= max(abs(dx), abs(dy));

  dx:= dx / t1;
  dy:= dy / t1;
  dz:= dz / t1;

  t:= 0;

  while t <= t1 do
    begin
      nx:= Round(t*dx) + x1;
      ny:= Round(t*dy) + y1;
      CutAt(nx,ny,n1.z + t*dz);
      inc(t);
    end;
 end;

procedure TSimulatorDlg.InitRender;
var
  E: TExtents;
  ExtX,ExtY,ExtZ: double;
  ExtMax: Double;
  x,y: integer;
begin
  Initialized:= False;
  if not Renderer.GetExtents(E) then
    begin
      writeln('3D- preview: nothing to draw!');
      Exit;
    end;
  ExtX:= (E.maxX - E.minX); // + AToolDia + 0.2;
  ExtY:= (E.maxY - E.minY); // + AToolDia + 0.2;;
  ExtZ:= (E.maxZ - E.minZ);
  ExtMax:= 1;
  if ExtX > ExtMax then ExtMax:= ExtX;
  if ExtY > ExtMax then ExtMax:= ExtY;
  Res:= ExtMax / SampleRate;
  writeln('Sample-Res:' + FloatToStrF(Res,ffFixed,8,4));
  // set initial Tool
  CreateTool(DefaultToolDia);
  writeln('Toolsize: ' + IntToStr(ToolDef.Size));
  CountX:= Round(Extx / Res);
  CountY:= Round(Exty / Res);
  if CountX < 1 then CountX:= 1;
  if CountY < 1 then CountY:= 1;
  MinX:= E.MinX;
  MinY:= E.MinY;
  writeln('Array Dimensions: ' + IntToStr(CountX) + 'x' + IntToStr(CountY));
  SetLength(NA,CountX,CountY);
  for x:= 0 to CountX - 1 do
    for y:= 0 to CountY - 1 do
      NA[X,Y]:= E.MaxZ;
  LightPosition[0]:= E.MinX + (ExtX/ 2);
  LightPosition[1]:= E.MinY + (ExtY / 2);
  LightPosition[2]:= E.MaxZ + ExtMax;
  LightPosition[3]:= 1; // position light (0 = directional light)
  initialized:= True;
end;

procedure triangle(x1,y1,z1,x2,y2,z2,x3,y3,z3: double);
var
  dx1,dy1,dz1,dx2,dy2,dz2: double;
  nx,ny,nz: double;
begin
  dx1 := x2-x1;
  dx2 := x3-x1;
  dy1 := y2-y1;
  dy2 := y3-y1;
  dz1 := z2-z1;
  dz2 := z3-z1;
  nx := dy1 * dz2 - dz1 * dy2;
  ny := dz1 * dx2 - dx1 * dz2;
  nz := dx1 * dy2 - dy1 * dx2;
  glNormal3f(nx, ny, nz);
  glVertex3f(x3,y3,z3);
  glVertex3f(x2,y2,z2);
  glVertex3f(x1,y1,z1);
end;

procedure TSimulatorDlg.ArrayToPos(i,k: integer; var x,y: double);
begin
  x:= (i * Res) + MinX;
  y:= (k * Res) + MinY;
end;

procedure TSimulatorDlg.MakeList;
var
  i,k:integer;
  x1,y1,x2,y2,x3,y3: double;
  z1,z2,z3: double;
  tc: integer;
//const
  //mat_ambient: Array[0..3] of glFLoat = (0.2, 0.2, 0.2,0);
  //light_amb: Array[0..3] of glFloat = (0.4,0.3,0.2,1);
  //light_dif: Array[0..3] of glFloat = (0.8,0.8,0.6,0);
begin
  if not Initialized then
    raise Exception.Create('Makelist called before init.');

  LabelStatus.Caption:= 'building triangles...';
  ProgressBar.Max:= CountX * CountY;
  ProgressBar.Position:= 0;

  glDeleteLists(GlList,1);
  glNewList(GlList,GL_COMPILE);
  glEnable(GL_NORMALIZE);
  glbegin(gl_triangles);
  for i:=0 to CountX - 2 do
    for k := 0 to CountY -2 do
      begin
        ArrayToPos(i,k,x1,y1);
        ArrayToPos(i+1,k,x2,y2);
        ArrayToPos(i+1,k+1,x3,y3);
        z1:= NA[i,k];
        z2:= NA[i+1,k];
        z3:= NA[i+1,k+1];
        triangle(x3,y3,z3,x2,y2,z2,x1,y1,z1);
        ArrayToPos(i,k,x1,y1);
        ArrayToPos(i+1,k+1,x2,y2);
        ArrayToPos(i,k+1,x3,y3);
        z1:= NA[i,k];
        z2:= NA[i+1,k+1];
        z3:= NA[i,k+1];
        triangle(x3,y3,z3,x2,y2,z2,x1,y1,z1);
      end;
  glend;
  glDisable(GL_NORMALIZE);
  glEndList;
  writeln('...finished!');
  LabelStatus.Caption:= 'Finished!';
end;

procedure TSimulatorDlg.CreateTool(ADia: Single);
var
  i: integer;
  x,y: integer;
  r: integer;
begin
  r:= Round((ADia/2) / Res);
  if r < 1 then r:= 1;
  i:= r*2;
  if i > 1024 then raise
    Exception.Create('Tool is too big.');
  if ToolDef.PA <> nil then
    FreeMem(ToolDef.PA);
  ToolDef.PA:= nil;
  ToolDef.Size:= i;
  GetMem(ToolDef.PA,SizeOf(Integer) * (i + 1));
  if ToolDef.PA = nil then
    raise Exception.Create('Not enough memory to create tool');
  with ToolDef do
    begin
      PA^[0]:= r;
      for y:= 0 to r do
        begin
          x:= Round(sqrt((r*r) - (y*y)));
          if x > r then x:= r;
          PA^[y]:= x;
          PA^[-y]:= x;
          // writeln('Test: ',x);
        end;
    end;
end;

procedure TSimulatorDlg.Render;
var
  P: PListItem;
  i: integer;
  tc: integer;
begin
  Initialized:= False;
  BtnRender.Enabled:= False;
  InitRender;
  if not Initialized then
    Exit;
  ProgressBar.Max:= Renderer.GetCount;
  Rendering:= True;
  Renderer.First;
  P:= Renderer.Get;
  tc:= 0;
  LabelStatus.Caption:= 'cutting along toolpath...';
  i:= 0;
  while P <> nil do
    begin
      if P^.ltype = ltTool then
        SetTool(P^.l1.x)
      else
      if P^.ltype <> ltDwell then
        CutLine(P^.l1,P^.l2);
      inc(i);
      inc(tc);
      if tc > 50 then
        begin
          tc:= 0;
          ProgressBar.Position:= i;
          Application.ProcessMessages;
        end;
      P:= Renderer.Get;
    end;
  //Lightning;
  MakeList;
  Rendering:= False;
  if DoAbort then
    ModalResult:= mrCancel
  else
    ModalResult:= mrOk;
  Close;
end;

procedure TSimulatorDlg.UpdateEdit;
var
  i: integer;
  s: string;
  v: single;
begin
  s:= EditTool.Text;
  i:= Pos(',',s);
  if i > 0 then s[i]:= '.';
  try
    v:= StrToFloat(s);
    if (v <= 0) then raise
      Exception.Create('Invalid Tool Diameter');
    if Vars.Metric then
      v:= v / 25.4;
    DefaultToolDia:= v;
  except
    on E: Exception do
      begin
        DefaultToolDia:= 0;
        ShowMessage(E.Message);
        EditTool.SetFocus;
      end;
  end;
end;

procedure TSimulatorDlg.SetTool(ADia: Double);
var
  i: integer;
begin
  i:= Round(ADia / Res);
  if i < 2 then i:= 2;
  if i <> ToolDef.Size then
    CreateTool(ADia);
  writeln('render tool: ', ToolDef.Size);
end;

procedure TSimulatorDlg.FormCreate(Sender: TObject);
begin
  DoAbort:= False;
  Initialized:= False;
  Rendering:= False;
  SampleRate:= 100;
  DefaultToolDia:= 0;
end;

procedure TSimulatorDlg.FormDestroy(Sender: TObject);
begin
  initialized:= False;
  SetLength(NA,0,0);
  if ToolDef.PA <> nil then
    FreeMem(ToolDef.PA);
end;

procedure TSimulatorDlg.BtnRenderClick(Sender: TObject);
begin
  UpdateEdit;
  if cbSample.ItemIndex = 0 then SampleRate:= 100 else
  if cbSample.ItemIndex = 1 then SampleRate:= 200 else
    SampleRate:= 300;
  Render;
end;

procedure TSimulatorDlg.BtnCancelClick(Sender: TObject);
begin
  DoAbort:= True;
end;

initialization
  {$I simulator.lrs}

end.

