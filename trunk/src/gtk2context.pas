...
uses
  Classes, SysUtils, LCLProc, LCLType, XUtil, XLib, gl, InterfaceBase,
  WSLCLClasses,
...
procedure gtk_gl_area_init(gl_area: PGTypeInstance; theClass: gpointer); cdecl;
begin
...
function context_new(visual: PGdkVisual; attrlist: plongint): PGdkGLContext;
var
  vi: PXVisualInfo;
...
begin
  Result:=nil;
  if visual = nil then ;
  vi:=glXChooseVisual(GDK_DISPLAY, DefaultScreen(GDK_DISPLAY), @attrList[0]);
  if vi = nil then
    raise Exception.Create('gdk_gl_context_new no visual found');
  glxcontext := glXCreateContext(GDK_DISPLAY, vi, nil, direct);
  XFree(vi);
  if (glxcontext = nil) then exit;
  PrivateContext := g_new(SizeOf(TGdkGLContextPrivate), 1);
  PrivateContext^.xdisplay:= GDK_DISPLAY;
...
begin
  Result := nil;
  visual := nil;
  glcontext:= context_new(visual,attrlist);
...
function make_current(drawable: PGdkDrawable; context: PGdkGLContext): boolean;
var
  PrivateContext: PGdkGLContextPrivate;
begin
  Result:=false;
  if  (drawable = nil) or (context = nil) then
    Exit;
  PrivateContext := PGdkGLContextPrivate(context);
  Result:= Boolean(glXMakeCurrent(PrivateContext^.xdisplay,
    GDK_WINDOW_XWINDOW(drawable),PrivateContext^.glxcontext)
...
begin
  ...
  Result:= make_current(PGtkWidget(glarea)^.window, glarea^.glcontext);
end;
...
begin
  ...
  drawable:= GTK_WIDGET(gl_area)^.window;
...
begin
  ...
  if not GTK_WIDGET_REALIZED(Widget) then
    Exit;
  if Size = nil then ;
